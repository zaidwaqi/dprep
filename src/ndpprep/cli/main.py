...
from ndpprep.refactor_pipeline.main import main as pipeline_main
...
def put(db, ds, batch, overwrite, secretpath):
    if secretpath is None:
        click.echo(
            f"Secret key not provided. Using default secret key: {DEFAULT_SECRET_PATH}"
        )
        secretpath = DEFAULT_SECRET_PATH
    try:
        if db == "raw_neps.db":
            process_neps_dataset(
                db, ds, batch, secretpath, TARGET_USER, TARGET_IP, TARGET_PATH
            )
        ...
        if overwrite:
            click.echo(
                f"Data from {db}/{ds}/{batch} has been put to SFTP server with overwrite."
            )
        else:
...
def shp(batch, secretpath):
    ...
    try:
        ...
        click.echo(
            f"Shapefile data for {batch} from EdgeNode has been saved in {shp} on CDSW/DMZ SFTP."
        )
    except Exception as e:
...
def get(db, ds, batch, secretpath):
    ...
    try:
        ...
        click.echo(
            f"Data from SFTP server has been saved and decrypted {db}/{ds}/{batch} on CDSW."
        )
    except Exception as e:
...
def unmask(db, ds, batch, secretpath):
    ...
    if secretpath is None:
        ...
        secretpath = DEFAULT_SECRET_PATH
    try:
        decrypted_tables, decrypted_files = decrypt_table(
            db, ds, batch, secretpath, TARGET_USER, TARGET_IP
        )
        unmask_sensitive(decrypted_tables, decrypted_files, mapping_table_folder)
        click.echo(
            f"Data from SFTP server has been saved and unmasked {db}/{ds}/{batch} on CDSW."
        )
    except Exception as e:
